...
<div id="page_wrap">	  
  ...
  <div class="center_content">
            ...
            <h1>Achizitionare bilet</h1>
            </div>
				<?php
					if (isset($_POST["inregistreaza"]))
						{
							$idutilizator = $_POST["iduser"];
							$idofertazbor = $_POST["idofertazbor"];
							$dataachizitie = date('Y-m-d', time());
							$dataluna = $_POST["dataluna"];
							$dataziua = $_POST["dataziua"];
							if ($dataluna == "-" or $dataziua == "-")
								{
									printf("<div class=\"section_full\">");
									printf("<h3>Data incorecta!</h3>");
									printf("<img src=\"images/error.png\" width=\"250\" height=\"200\"><a href=\"achizitionare.php?id=%s&uid=%s\" class=\"order_button\">Inapoi</a>",$idofertazbor,$idutilizator);
									printf("</div>");
								}
							else
								{
									$dataplecare = "2013-" . $dataluna . "-" . $dataziua;
									InterogareSQL("insert into pf_biletezbor(idutilizator,dataachizitie,dataplecare,idofertazbor) values(".$idutilizator.",'".$dataachizitie."','".$dataplecare."',".$idofertazbor.");",$rez1);
									printf("<div class=\"section_full\">");
									printf("<h3>Achizitionare bilet efectuata!</h3>");
									printf("<a href=\"achizitionare.php?id=%s&uid=%s\" class=\"order_button\">Inapoi</a>",$idofertazbor,$idutilizator);
									printf("</div>");
								}
						}
					else
							...
							else
							{
								$idof=$_GET["id"];
								$idusr=$_GET["uid"];
								printf("<form name=\"f1\" method=\"post\" action=\"achizitionare.php\">");
								if (($nr1 = InterogareSQL("select c.idofertazbor,a.nume,b.nume,c.pret,c.duratazbor,d.numecompanie from pf_localitati a,pf_localitati b,pf_ofertebiletezbor c,pf_companii d where a.idlocalitate=c.idlocplecare and b.idlocalitate=c.idlocsosire and c.idcompanie=d.idcompanie and c.idofertazbor=".$idof.";",$rez1)) != 0)
									{
										...
										printf("</div>");
										printf("<div class=\"form_row\">");
										printf("<label>Durata zbor:</label>");
										printf("<input type=\"text\" class=\"form_val\" name=\"durata\" value=\"%sh:%smin\" readonly/>",$ore,$minute);
										printf("</div>");
										printf("<div class=\"form_row\">");
										printf("<label>Transport prin:</label>");
										printf("<input type=\"text\" class=\"form_val\" name=\"companie\" value=\"%s\" readonly/>",$linie1[5]);
										printf("</div>");
										printf("<div class=\"form_row\">");
										...
										printf("</div>");
										printf("<input type=\"hidden\" name=\"idofertazbor\" value=\"%s\">",$linie1[0]);
										printf("</div>");
									}
